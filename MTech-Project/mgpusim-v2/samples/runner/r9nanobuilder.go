...
import (
	...
	"gitlab.com/akita/mgpusim/v2/samples/runner/utility/crossbar"
...
type R9NanoGPUBuilder struct {
	...
	l1Crossbars []*crossbar.Crossbar //
}

var LogColorConnection = logger.Blue //CODE
...
func (b *R9NanoGPUBuilder) connectL1ToL2() {
	...
	if b.configs.isL1VCacheVIVT { //CODE
...
func (b *R9NanoGPUBuilder) connectCPWithTLBs() {
	...
	for _, tlb := range b.l1vTLBs {
		ctrlPort := tlb.GetPortByName("Control")
		b.cp.TLBs = append(b.cp.TLBs, ctrlPort)
		b.internalConn.PlugIn(ctrlPort, b.configs.numTLBsPerAggregatedL1VTLB) //CODE
	}
...
// WithConfigs sets the new TLB configurations
func (b R9NanoGPUBuilder) WithConfigs(configs Configs) R9NanoGPUBuilder {
	b.configs = configs
	return b
}

func (b *R9NanoGPUBuilder) buildCrossbars() {
	if b.configs.numAggregatedL1VTLBs == 64 && b.configs.numClusters == 64 {
		logger.PrintLog(false, "\n[LOG] No crossbar needed for default configuration.", logger.Yellow)
		return
	}

	numL1Crossbars := b.configs.numClusters
	numl1CrossbarInputs := (64 / b.configs.numClusters)
	numl1CrossbarOutputs := (b.configs.numAggregatedL1VTLBs / b.configs.numClusters)

	logger.PrintLog(false, fmt.Sprintf("\n[LOG] %d L1 crossbars %d X %d", numL1Crossbars, numl1CrossbarInputs, numl1CrossbarOutputs), logger.Yellow)

	for i := 0; i < numL1Crossbars; i++ {
		builder := crossbar.MakeBuilder().
			WithEngine(b.engine).
			WithFreq(b.freq).
			WithNumInputPorts(numl1CrossbarInputs).
			WithNumOutputPorts(numl1CrossbarOutputs).
			WithNumSetsPerTLB(b.configs.numTLBsPerAggregatedL1VTLB) //This needs to be changed

		l1CrossbarName := fmt.Sprintf("%s.L1_Crossbar_%02d", b.gpuName, i)
		l1Crossbar := builder.Build(l1CrossbarName)

		l1Crossbar.PopulateATPorts(i, b.l1vAddrTrans)
		l1Crossbar.PopulateTLBPorts(i, b.l1vTLBs)

		b.l1Crossbars = append(b.l1Crossbars, l1Crossbar)

		if b.monitor != nil {
			b.monitor.RegisterComponent(l1Crossbar)
		}
	}
}

func (b *R9NanoGPUBuilder) buildL1VAggregatedTLBs() {
	builder := tlb.MakeBuilder().
		WithEngine(b.engine).
		WithFreq(b.freq).
		WithNumSets(b.configs.numTLBsPerAggregatedL1VTLB).
		WithNumWays(128).
		WithNumMSHREntry(4 * b.configs.numTLBsPerAggregatedL1VTLB).
		WithNumReqPerCycle(4)

	for i := 0; i < b.configs.numAggregatedL1VTLBs; i++ {
		tlbName := fmt.Sprintf("%s.L1VTLB_%02d", b.gpuName, i)
		tlbLevel := 1
		tlb := builder.Build(tlbName, tlbLevel)
...
func (b *R9NanoGPUBuilder) buildL2TLB() {
	...
	tlbName := fmt.Sprintf("%s.L2TLB", b.gpuName)
	tlbLevel := 2
	l2TLB := builder.Build(tlbName, tlbLevel)
...
func (b *R9NanoGPUBuilder) connectTLBs() {
	b.connectATtoL1TLB()
	b.connectL1TLBToL2TLB()
}

func (b *R9NanoGPUBuilder) connectATtoL1TLB() {
	if b.configs.numAggregatedL1VTLBs == 64 && b.configs.numClusters == 64 {
		for i := 0; i < 64; i++ {
			tlb := b.l1vTLBs[i]
			at := b.l1vAddrTrans[i]
			...
			b.connectWithDirectConnection(atTranslationPort, tlbTopPort, 8)

			logger.PrintLog(false, fmt.Sprintf("\n[LOG] [%s] => [%s]", atTranslationPort.Name(), tlbTopPort.Name()), LogColorConnection)
		}
	} else {
		atIndex := 0
		tlbIndex := 0

		for _, l1Crossbar := range b.l1Crossbars {
			for _, inputPort := range l1Crossbar.InputPorts {
				at := b.l1vAddrTrans[atIndex]
				atTranslationPort := at.GetPortByName("Translation")

				at.SetTranslationProvider(inputPort)
				b.connectWithDirectConnection(atTranslationPort, inputPort, 8)
				atIndex++

				logger.PrintLog(false, fmt.Sprintf("\n[LOG] [%s] => [%s]", atTranslationPort.Name(), inputPort.Name()), LogColorConnection)
			}

			for _, outputPort := range l1Crossbar.OutputPorts {
				tlb := b.l1vTLBs[tlbIndex]
				tlbTopPort := tlb.GetPortByName("Top")

				b.connectWithDirectConnection(outputPort, tlbTopPort, 8*b.configs.numTLBsPerAggregatedL1VTLB)
				tlbIndex++

				logger.PrintLog(false, fmt.Sprintf("\n[LOG] [%s] => [%s]", outputPort.Name(), tlbTopPort.Name()), LogColorConnection)
			}
		}
	}
}
...
func (b *R9NanoGPUBuilder) connectL1TLBToL2TLB() {
	tlbConn := sim.NewDirectConnection(b.gpuName+"L1TLB-L2TLB", b.engine, b.freq)

	l2TLBTopPort := b.l2TLBs[0].GetPortByName("Top")
	tlbConn.PlugIn(l2TLBTopPort, 64)

	for _, l1iTLB := range b.l1iTLBs {
		l1iTLB.LowModule = l2TLBTopPort
		tlbConn.PlugIn(l1iTLB.GetPortByName("Bottom"), 16)
	}

	for _, l1sTLB := range b.l1sTLBs {
		l1sTLB.LowModule = l2TLBTopPort
		tlbConn.PlugIn(l1sTLB.GetPortByName("Bottom"), 16)
	}

	for _, l1vTLB := range b.l1vTLBs {
		l1vTLB.LowModule = l2TLBTopPort
		tlbConn.PlugIn(l1vTLB.GetPortByName("Bottom"), 16*b.configs.numTLBsPerAggregatedL1VTLB)
	}
}
...
//Veriyify Code:
1) Configs
2) Build - TLB, L1VCache
3) Connect - Vector Memory, AT-L1TLB-L2TLB, Cache
4) Component - TLB, Crossbars
5) Metrics - Stats calclulate, Mememory consumed
*/
