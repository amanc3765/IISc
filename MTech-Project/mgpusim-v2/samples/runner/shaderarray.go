...
func (b *shaderArrayBuilder) buildL1STLB(sa *shaderArray) {
	...
	name := fmt.Sprintf("%s.L1STLB", b.name) //CODE
	tlbLevel := 1                            //
	tlb := builder.Build(name, tlbLevel)     //
	sa.l1sTLB = tlb
...
func (b *shaderArrayBuilder) buildL1ITLB(sa *shaderArray) {
	...
	name := fmt.Sprintf("%s.L1ITLB", b.name) //CODE
	tlbLevel := 1                            //
	tlb := builder.Build(name, tlbLevel)     //
	sa.l1iTLB = tlb
...
func (b shaderArrayBuilder) withConfigs(configs Configs) shaderArrayBuilder {
	b.configs = configs
	return b
}
...
func (b *shaderArrayBuilder) connectVectorMem(sa *shaderArray) {
	for i := 0; i < b.numCU; i++ {
		...
		atTopPort := at.GetPortByName("Top")
		l1vTopPort := l1v.GetPortByName("Top")

		if b.configs.isL1VCacheVIVT {
			//ROB => L1V$ =>AT

			rob.BottomUnit = l1vTopPort
			b.connectWithDirectConnection(rob.GetPortByName("Bottom"), l1vTopPort, 8)

			l1v.SetLowModuleFinder(&mem.SingleLowModuleFinder{
				LowModule: atTopPort,
			})
			b.connectWithDirectConnection(l1v.GetPortByName("Bottom"), atTopPort, 8)

			logger.PrintLog(false, fmt.Sprintf("\n[LOG] ROB_Bottom[%s]   L1$_Bottom[%s] ", rob.BottomUnit.Name(), atTopPort.Name()), LogColorConnection)
		} else {
			//ROB => AT => L1V$
			...
			at.SetLowModuleFinder(&mem.SingleLowModuleFinder{
				LowModule: l1vTopPort,
			})
			b.connectWithDirectConnection(at.GetPortByName("Bottom"), l1vTopPort, 8)

			logger.PrintLog(false, fmt.Sprintf("\n[LOG] ROB_Bottom[%s]   AT_Bottom[%s] ", rob.BottomUnit.Name(), l1vTopPort.Name()), LogColorConnection)
		}
	}
}
