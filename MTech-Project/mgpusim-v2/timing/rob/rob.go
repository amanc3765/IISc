...
type transaction struct {
	...
	timeStart     sim.VTimeInSec
	timeEnd       sim.VTimeInSec
}
...
func (b *ReorderBuffer) parseBottom(now sim.VTimeInSec) bool {
	...
	if found {
		...
		// req := trans.reqFromTop
		// cycles := b.Freq.Cycle(trans.timeEnd) - b.Freq.Cycle(trans.timeStart)

		// if strings.Contains(b.Name(), "VROB") {
		// 	fmt.Printf("\nNum of cycles for request (%d %d) : %d %s", req.GetAddress(), req.GetByteSize(), cycles, b.Name())
		// }
