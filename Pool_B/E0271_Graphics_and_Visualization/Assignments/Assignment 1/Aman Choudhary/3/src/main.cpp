...
void onDisplay()
{
	...
	trans->axis = Vector3f(1.0f, 0.0f, 0.0f);
	trans->angle = ToRadian(-30);
	trans->setRotate();

	int uf_uf_rotate30_location = glGetUniformLocation(shaderProgram, "uf_rotate30");
	glUniformMatrix4fv(uf_uf_rotate30_location, 1, GL_TRUE, &((trans->rotateMat).m[0][0]));

	trans->setIdentity();
	trans->axis = Vector3f(0.0f, 1.0f, 0.0f);
