...
extern unsigned int minScalarVal, maxScalarVal;
...
ScalarField *readScalarFile(char *scalarFile) {
  ...
  //       fscanf(input, "%d", &val);
  //       index = (currZ * (scalarField->side * scalarField->side)) +
  //               (currY * scalarField->side) + currX;
...
void printScalarField(ScalarField *scalarField, int numPoints) {
  ...
  for (int i = 0; i < numPoints; ++i) {
    cout << setw(10) << (scalarField->points[i]).position.x << " " << setw(10)
         << (scalarField->points[i]).position.y << " " << setw(10)
         << (scalarField->points[i]).position.z << " " << setw(10)
         << (scalarField->points[i]).scalarVal << endl;
  }
}